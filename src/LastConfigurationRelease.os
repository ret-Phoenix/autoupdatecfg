...
//
// Параметры:
//  ПутьКНастройкам - Строка - Путь к файлу настроек
// Возвращаемое значение:
//  Соответствие - Параметры авторизации
//
Функция ПолучитьПараметрыДоступаНаСайтИТС(ПутьКНастройкам="") Экспорт

	Если ПутьКНастройкам = "" Тогда
		КаталогСкрипта = Новый Файл(СтартовыйСценарий().Источник).Путь;
		ПутьКНастройкам = КаталогСкрипта + "its.json";
	КонецЕсли;
	ФайлПараметров = Новый Файл(ПутьКНастройкам);

	Если НЕ ФайлПараметров.Существует() Тогда
		ВызватьИсключение "Нет файла настроек для доступа на сайт ИТС (its.json)";
	КонецЕсли;

	ЧтениеТекста = Новый ЧтениеТекста(ПутьКНастройкам, "utf-8");
